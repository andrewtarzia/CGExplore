...
def data_to_array(json_files, output_dir):
    ...
    for j_file in json_files:
        ...
        if t_str in cage_topology_options(
            ...
            row["c2r0"] = get_cgbead_from_type(
                present_c2_beads[0],
                core_2c_beads(),
            ).bond_r
            row["c2angle"] = get_cgbead_from_type(
                present_c2_beads[1],
                arm_2c_beads(),
            ).angle_centered
            ...
            cltitle = "4C1"
            row["c3r0"] = get_cgbead_from_type(
                present_c2_beads[0],
                cl_bead_libs,
            ).bond_r
            row["c3angle"] = get_cgbead_from_type(
                present_c2_beads[0],
                cl_bead_libs,
            ).angle_centered

        row["cltitle"] = cltitle
        row["clr0"] = get_cgbead_from_type(
            present_cl_beads[0],
            cl_bead_libs,
        ).bond_r
        row["clangle"] = clangle
